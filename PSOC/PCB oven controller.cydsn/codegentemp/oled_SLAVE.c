...
volatile uint8 oled_slStatus;   /* Slave Status  */

/* Transmit buffer variables */
volatile uint8 * oled_slRdBufPtr;   /* Pointer to Transmit buffer  */
volatile uint8   oled_slRdBufSize;  /* Slave Transmit buffer size  */
volatile uint8   oled_slRdBufIndex; /* Slave Transmit buffer Index */

/* Receive buffer variables */
volatile uint8 * oled_slWrBufPtr;   /* Pointer to Receive buffer  */
volatile uint8   oled_slWrBufSize;  /* Slave Receive buffer size  */
volatile uint8   oled_slWrBufIndex; /* Slave Receive buffer Index */

#if (oled_SW_ADRR_DECODE)
    volatile uint8 oled_slAddress;  /* Software address variable */
#endif /* (oled_SW_ADRR_DECODE) */


/*******************************************************************************
* Function Name: oled_SlaveStatus
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  oled_slStatus - The global variable used to store a current
*                              status of the I2C slave.
*
*******************************************************************************/
uint8 oled_SlaveStatus(void) 
{
    return (oled_slStatus);
}


/*******************************************************************************
* Function Name: oled_SlaveClearReadStatus
********************************************************************************
*
* Summary:
*  Clears the read status flags and returns they values.
*  The oled_SSTAT_RD_BUSY flag is not effected by clear.
*
* Parameters:
*  None.
*
...
uint8 oled_SlaveClearReadStatus(void) 
{
    ...
    status = (oled_slStatus & oled_SSTAT_RD_MASK);
    oled_slStatus &= (uint8) ~oled_SSTAT_RD_CLEAR;
...
/*******************************************************************************
* Function Name: oled_SlaveClearWriteStatus
********************************************************************************
*
* Summary:
*  Clears the write status flags and returns they values.
*  The oled_SSTAT_WR_BUSY flag is not effected by clear.
*
* Parameters:
*  None.
*
...
uint8 oled_SlaveClearWriteStatus(void) 
{
    ...
    /* Mask of transfer complete flag and Error status */
    status = (oled_slStatus & oled_SSTAT_WR_MASK);
    oled_slStatus &= (uint8) ~oled_SSTAT_WR_CLEAR;
...
/*******************************************************************************
* Function Name: oled_SlaveSetAddress
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  oled_Address  - The global variable used to store an I2C slave
...
{
#if (oled_SW_ADRR_DECODE)
    oled_slAddress = (address & oled_SLAVE_ADDR_MASK);
#else
    oled_ADDR_REG  = (address & oled_SLAVE_ADDR_MASK);
#endif /* (oled_SW_ADRR_DECODE) */
}


/*******************************************************************************
* Function Name: oled_SlaveInitReadBuf
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  oled_slRdBufPtr   - The global variable used to store a pointer
*                                  to the slave read buffer.
*  oled_slRdBufSize  - The global variable used to store a slave
*                                  read buffer size.
*  oled_slRdBufIndex - The global variable used to store a current
     ...
     
{
    if (NULL != rdBuf)
    {
        ...
        /* Set buffer pointer */
        oled_slRdBufPtr   = (volatile uint8 *) rdBuf;
        oled_slRdBufSize  = bufSize;    /* Set buffer size     */
        oled_slRdBufIndex = 0u;         /* Clears buffer index */
...
/*******************************************************************************
* Function Name: oled_SlaveInitWriteBuf
********************************************************************************
*
* Summary:
*  Sets the buffer pointer and size of the read buffer. This function also
...
*
* Global variables:
*  oled_slWrBufPtr   - The global variable used to store a pointer
*                                  to the slave write buffer.
*  oled_slWrBufSize  - The global variable used to store a slave
*                                  write buffer size.
*  oled_slWrBufIndex - The global variable used to store a current
     ...
     
{
    if (NULL != wrBuf)
    {
        oled_DisableInt(); /* Lock from interrupt */
         ...
         /* Set buffer pointer */
        oled_slWrBufPtr   = (volatile uint8 *) wrBuf;
        oled_slWrBufSize  = bufSize;    /* Set buffer size     */
        oled_slWrBufIndex = 0u;         /* Clears buffer index */
...
/*******************************************************************************
* Function Name: oled_SlaveGetReadBufSize
********************************************************************************
*
* Summary:
...
{
    return (oled_slRdBufIndex);
}


/*******************************************************************************
* Function Name: oled_SlaveGetWriteBufSize
********************************************************************************
*
* Summary:
...
{
    return (oled_slWrBufIndex);
}


/*******************************************************************************
* Function Name: oled_SlaveClearReadBuf
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: oled_SlaveClearRxBuf
********************************************************************************
*
* Summary:
